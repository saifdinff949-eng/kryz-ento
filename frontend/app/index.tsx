...
import { sendOTP, verifyOTP, registerUser } from '../src/services/api';
...
export default function LoginScreen() {
  ...
  const handleSendOTP = async () => {
    ...
    try {
      ...
      const msg = error.response?.data?.detail || 'فشل إرسال رمز التحقق. حاول مرة أخرى.';
      Alert.alert('خطأ', msg);
    } finally {
      setIsLoading(false);
    }
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      const msg = error.response?.data?.detail || 'فشل التسجيل. حاول مرة أخرى.';
      Alert.alert('خطأ', msg);
    } finally {
      setIsLoading(false);
    }
