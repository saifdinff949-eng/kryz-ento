...
export default function SharesScreen() {
  ...
  const handleSubmit = async () => {
    ...
    if (!selectedPackage) {
      Alert.alert('خطأ', 'يرجى اختيار عدد الشير');
      return;
    }
  ...
  return (
    <LinearGradient colors={bgColors} style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={[styles.headerTitle, { color: textColor }]}>الشير</Text>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
          ...
          {PACKAGES.map((pkg) => {
            ...
            return (
              <TouchableOpacity
                ...
                disabled={!canAfford}
              >
                <Text style={[styles.packageQuantity, { color: isSelected ? '#10B981' : textColor }]}>
                  {pkg.quantity}
                </Text>
        ...
        >
          <LinearGradient
            colors={postUrl && selectedPackage ? ['#10B981', '#059669'] : ['#6B7280', '#4B5563']}
            start={{ x: 0, y: 0 }}
            end={{ x: 1, y: 0 }}
      ...
      <Modal visible={showModal} transparent animationType="fade">
        <View style={styles.modalOverlay}>
          <View style={[styles.modalContent, { backgroundColor: isDark ? '#1A1A2E' : '#FFF' }]}>
            {step === 'searching' && (
              <>
                <Ionicons name="search" size={50} color="#10B981" />
                <Text style={[styles.modalTitle, { color: textColor }]}>جاري البحث</Text>
                <View style={styles.progressContainer}>
                  <Animated.View style={[styles.progressBar, { width: progressWidth, backgroundColor: '#10B981' }]} />
                </View>
            ...
            {step === 'confirm' && (
              <>
                <Ionicons name="share-social" size={60} color="#10B981" />
...
const styles = StyleSheet.create({
  ...
  packageSelected: { borderColor: '#10B981' },
