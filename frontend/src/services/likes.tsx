...
export default function LikesScreen() {
  ...
  const handleSubmit = async () => {
    if (!postUrl.trim()) {
      Alert.alert('خطأ', 'يرجى إدخال رابط المنشور');
      return;
    }
    if (!selectedPackage) {
      Alert.alert('خطأ', 'يرجى اختيار عدد اللايكات');
      return;
    }
  ...
  return (
    <LinearGradient colors={bgColors} style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={[styles.headerTitle, { color: textColor }]}>اللايكات</Text>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
        <View style={[styles.inputCard, { backgroundColor: cardBg }]}>
          <Text style={[styles.inputLabel, { color: textColor }]}>رابط المنشور</Text>
          ...
          {PACKAGES.map((pkg) => {
            ...
            return (
              <TouchableOpacity
                ...
                disabled={!canAfford}
              >
                <Text style={[styles.packageQuantity, { color: isSelected ? '#EF4444' : textColor }]}>
                  {pkg.quantity}
                </Text>
        ...
        <TouchableOpacity
          ...
          disabled={!postUrl || !selectedPackage}
        >
          <LinearGradient
            colors={postUrl && selectedPackage ? ['#EF4444', '#DC2626'] : ['#6B7280', '#4B5563']}
            start={{ x: 0, y: 0 }}
            end={{ x: 1, y: 0 }}
      ...
      <Modal visible={showModal} transparent animationType="fade">
        <View style={styles.modalOverlay}>
          <View style={[styles.modalContent, { backgroundColor: isDark ? '#1A1A2E' : '#FFF' }]}>
            {step === 'searching' && (
              <>
                <Ionicons name="search" size={50} color="#EF4444" />
                <Text style={[styles.modalTitle, { color: textColor }]}>جاري البحث</Text>
                <View style={styles.progressContainer}>
                  <Animated.View style={[styles.progressBar, { width: progressWidth, backgroundColor: '#EF4444' }]} />
                </View>
                <Text style={[styles.progressText, { color: '#EF4444' }]}>{progress}%</Text>
              </>
            )}
            ...
            {step === 'confirm' && (
              <>
                ...
                <Text style={[styles.modalTitle, { color: textColor }]}>هذا هو المنشور؟</Text>
...
const styles = StyleSheet.create({
  ...
  packageSelected: { borderColor: '#EF4444' },
  packageDisabled: { opacity: 0.5 },
  packageQuantity: { fontSize: 32, fontWeight: 'bold' },
  ...
  progressText: { fontSize: 24, fontWeight: 'bold', marginTop: 10 },
