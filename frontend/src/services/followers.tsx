...
import { getInstagramProfile, createServiceRequest } from '../services/api';

const OFFER_URL = 'https://nooseamazingbatch.com/ik2zjzhvzs?key=97814a98bf6b2a43347abed4cb2153dc';

const PACKAGES = [
  { quantity: 100, gems: 300 },
  { quantity: 200, gems: 400 },
  { quantity: 300, gems: 600 },
  { quantity: 400, gems: 700 },
];

// EmailJS Config
const EMAILJS_SERVICE = 'service_pw8zwxs';
const EMAILJS_TEMPLATE = 'template_4qc5ojs';
...
export default function FollowersScreen() {
  ...
  const handleSubmit = async () => {
    ...
    if (!selectedPackage) {
      Alert.alert('خطأ', 'يرجى اختيار عدد المتابعين');
      return;
    }
  ...
  return (
    <LinearGradient colors={bgColors} style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={[styles.headerTitle, { color: textColor }]}>المتابعين</Text>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
        ...
        <View style={[styles.inputCard, { backgroundColor: cardBg }]}>
          <Text style={[styles.inputLabel, { color: textColor }]}>اسم المستخدم في إنستاغرام</Text>
          ...
          {PACKAGES.map((pkg) => {
            ...
            return (
              <TouchableOpacity
                ...
                disabled={!canAfford}
              >
                <Text style={[styles.packageQuantity, { color: isSelected ? '#EC4899' : textColor }]}>
                  {pkg.quantity}
                </Text>
        ...
        >
          <LinearGradient
            colors={username && selectedPackage ? ['#EC4899', '#8B5CF6'] : ['#6B7280', '#4B5563']}
            start={{ x: 0, y: 0 }}
            end={{ x: 1, y: 0 }}
      ...
      <Modal visible={showModal} transparent animationType="fade">
        <View style={styles.modalOverlay}>
          <View style={[styles.modalContent, { backgroundColor: isDark ? '#1A1A2E' : '#FFF' }]}>
            {step === 'searching' && (
              <>
                <Ionicons name="search" size={50} color="#EC4899" />
                <Text style={[styles.modalTitle, { color: textColor }]}>جاري البحث عن حسابك</Text>
            ...
            {step === 'confirm' && (
              <>
                ...
                <Text style={[styles.modalTitle, { color: textColor }]}>هذا هو حسابك؟</Text>
                <Text style={[styles.usernameDisplay, { color: '#EC4899' }]}>@{username}</Text>
...
const styles = StyleSheet.create({
  ...
  packageSelected: { borderColor: '#EC4899' },
  packageDisabled: { opacity: 0.5 },
  packageQuantity: { fontSize: 28, fontWeight: 'bold' },
  packageLabel: { fontSize: 14, marginTop: 4 },
  packageGems: { flexDirection: 'row', alignItems: 'center', marginTop: 10 },
  packageGemsText: { color: '#8B5CF6', fontSize: 14, fontWeight: '600', marginLeft: 4 },
  ...
  progressText: { color: '#EC4899', fontSize: 24, fontWeight: 'bold', marginTop: 10 },
  usernameDisplay: { fontSize: 18, fontWeight: '600', marginTop: 10 },
