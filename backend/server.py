...
async def send_otp(request: PhoneAuthRequest):
    ...
    await db.otp_codes.delete_many({"phone": request.phone})
    ...
    if twilio_client and TWILIO_FROM_NUMBER:
        try:
            message = twilio_client.messages.create(
                body=f"kryz en app - رمز التحقق: {otp_code}",
                from_=TWILIO_FROM_NUMBER,
                to=request.phone
            )
            logging.info(f"OTP sent to {request.phone}, SID: {message.sid}")
            return {"success": True, "message": "تم إرسال رمز التحقق"}
        except Exception as e:
            logging.error(f"Twilio SMS error: {e}")
            raise HTTPException(status_code=500, detail=f"فشل إرسال الرسالة: {str(e)}")
    else:
